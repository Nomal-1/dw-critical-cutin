 ...
 *
 * 주의: 캔버스를 다시 그리는 내부 메서드 이름을 "_render"로 짓지 말 것.
 * Application 클래스가 실제 창 렌더링에 쓰는 내부 메서드 이름과 겹쳐서
 * render(true)를 호출해도 창이 열리지 않는 문제가 있었다(_redrawCanvas로 회피).
 */
 ...
 * 프레임 이미지는 PNG 알파 채널은 있지만, 빈 공간이 완전 투명이 아니라
 * 회색(RGB 159,159,159)으로 불투명하게 칠해져 있다. 그래서 저장할 때
 * removeGrayMatte()로 그 회색을 색상 키잉(chroma keying)해서 투명하게
 * 지워준다 (그대로 두면 컷인 이미지 바깥이 회색 사각형으로 나온다).
 */
...
const OUTER_GRAY = { r: 159, g: 159, b: 159 };
const GRAY_TOLERANCE = 14;
...
export class CutinMaker extends Application {
  ...
  /**
   * 프레임 PNG는 빈 공간이 완전 투명이 아니라 회색(RGB 159,159,159)으로
   * 불투명하게 칠해져 있다. 화면 미리보기용 캔버스는 그대로 두고, 내보낼
   * 때만 별도 캔버스에 다시 그려서 그 회색과 비슷한 픽셀의 알파를 0으로
   * 만든다 (색상 키잉). 허용 오차(GRAY_TOLERANCE) 안의 색만 지운다.
   */
  _exportBlob() {
    ...
    for (let i = 0; i < px.length; i += 4) {
      const dr = px[i] - OUTER_GRAY.r;
      const dg = px[i + 1] - OUTER_GRAY.g;
      const db = px[i + 2] - OUTER_GRAY.b;
      if (Math.sqrt(dr * dr + dg * dg + db * db) <= GRAY_TOLERANCE) {
        px[i + 3] = 0;
      }
    }
